...
const ReusableFilter = ({ title, options }: IFilterProps) => {
  ...
  return (
    <div className="bg-[#FFF7EC] w-full lg:mx-auto border-none md:mx-2 lg:w-[286px]   rounded-xl  px-4 lg:px-[9px] py-3 mb-5 border-1 border-[#E5E5E5] ">
      <h3 className="font-medium text-xl text-[#1A1A1A] mb-2">{title}</h3>
      <ul className="space-y-2">
        {options.map((option) => (
          <li
            key={option.value}
            className="flex items-center   border-none  justify-between bg-white px-4 py-2 rounded  border-1 hover:bg-orange-100 transition"
          >
            ...
            <div className="inline-flex items-center justify-center w-8 h-8">
              <Checkbox
                ...
                className="w-5 border-[#FCAB3F] h-5 data-[state=checked]:border-[#FCAB3F] data-[state=checked]:bg-[#FCAB3F] data-[state=checked]:text-white cursor-pointer"
              />
            </div>
          </li>
        ))}
